;;;; tutorial-part13.lisp
;;;; "Adventure gear"
;;;; February 2017
;;;;
;;;; Commentary to be posted on 9bladed.com
;;;;
;;;; Original python+libtcod tutorial, part 13:
;;;; http://www.roguebasin.com/index.php?title=Complete_Roguelike_Tutorial,_using_python%2Blibtcod,_part_13
...
(defparameter +heal-amount+ 40)
(defparameter +lightning-damage+ 40)
...
(defparameter +fireball-damage+ 25)
...
(defun inventory-menu (header)
  ...
  (let (options index)
    (if (eql (length *inventory*) 0)
        ...
        (setf options (loop for item in *inventory*
...
(defgeneric get-all-equipped (obj)
  (:documentation "returns a list of equipped items"))

(defmethod get-all-equipped ((obj object))
  (when (eql obj *player*) ; other objects have no equipment
...
(defclass equipment (item)
  ((is-equipped
    :initarg :is-equipped
    :initform nil
    :accessor is-equipped)
   (equip-slot
    :initarg :equip-slot
    :initform (error "Must provide equipment slot")
    :accessor equip-slot)
   (power-bonus
    :initarg :power-bonus
    :initform 0
    :accessor power-bonus)
   (defense-bonus
    :initarg :defense-bonus
    :initform 0
    :accessor defense-bonus)
   (max-hp-bonus
    :initarg :max-hp-bonus
    :initform 0
    :accessor max-hp-bonus))
  (:documentation "an object that can be equipped, yielding bonuses. is also an item."))

(defgeneric toggle-equip (self)
  (:documentation "toggle equip/dequip status"))

(defgeneric equip (self)
  (:documentation "equip object and show a message about it"))

(defgeneric dequip (self)
  (:documentation "dequip object and show a message about it"))

(defmethod toggle-equip ((self equipment))
  (if (is-equipped self)
      (dequip self)
      (equip self)))

(defmethod equip ((self equipment))
  ;; if the slot is already being used, dequip whatever is there first
  (let ((old-equipment (get-equipped-in-slot (equip-slot self))))
    (if old-equipment
        (dequip old-equipment)))
  
  (setf (is-equipped self) t)
  (message (format nil "Equipped ~a on ~(~a~)." (name self) (equip-slot self)) :light-green))

(defmethod dequip ((self equipment))
  (when (is-equipped self)
    (setf (is-equipped self) nil)
    (message (format nil "Dequipped ~a from ~(~a~)." (name self) (equip-slot self)) :light-yellow)))

(defmethod pick-up :after ((thing equipment))
  "special case: automatically equip, if the corresponding equipment slot is unused"
  (unless (get-equipped-in-slot (equip-slot thing))
    (equip thing)))

(defmethod drop :before ((self equipment))
  "special case: if the object is equipment, dequip it before dropping"
  (dequip self))

(defmethod use ((self equipment))
  "special case: if the object is equipment, the 'use' action is to equip/dequip"
  (toggle-equip self))

(defun get-equipped-in-slot (eslot)
  "returns the equipment in a slot, or nil if it's empty"
  (dolist (obj *inventory*)
    (if (and (typep obj 'equipment) (eql (equip-slot obj) eslot) (is-equipped obj))
        (return-from get-equipped-in-slot obj))))
...
(defgeneric power (thing)
  (:documentation "read the power value including bonuses"))

(defmethod power ((thing fighter))
  (+ (slot-value thing 'power) (loop for equipment in (get-all-equipped thing)
                                     sum (power-bonus equipment) into bonus
                                     finally (if bonus
                                                 (return bonus)
                                                 (return 0)))))

(defgeneric (setf power) (value thng)
  (:documentation "set the base power"))

(defmethod (setf power) (value (thing fighter))
  (setf (slot-value thing 'power) value))

(defgeneric defense (thing)
  (:documentation "read the defense value including bonuses"))

(defmethod defense ((thing fighter))
  (+ (slot-value thing 'defense) (loop for equipment in (get-all-equipped thing)
                                       sum (defense-bonus equipment) into bonus
                                       finally (if bonus
                                                   (return bonus)
                                                   (return 0)))))

(defgeneric (setf defense) (value thng)
  (:documentation "set the base defense"))

(defmethod (setf defense) (value (thing fighter))
  (setf (slot-value thing 'defense) value))

(defgeneric max-hp (thing)
  (:documentation "read the max-hp value including bonuses"))

(defmethod max-hp ((thing fighter))
  (+ (slot-value thing 'max-hp) (loop for equipment in (get-all-equipped thing)
                                      sum (max-hp-bonus equipment) into bonus
                                      finally (if bonus
                                                   (return bonus)
                                                   (return 0)))))

(defgeneric (setf max-hp) (value thng)
  (:documentation "set the base max-hp"))

(defmethod (setf max-hp) (value (thing fighter))
  (setf (slot-value thing 'max-hp) value))

(defmethod initialize-instance :after ((f fighter) &key)
  ;; access max-hp directly on creation (inventory/player may not be defined yet)
  (setf (hp f) (slot-value f 'max-hp)))
...
(defun random-choice-index (chances)
  "choose one option from list of chances, returning its index"
  ;; the dice will land on some number between 1 and the sum of the chances
  (let ((dice (tcod:random-get-int tcod:+null+ 1 (apply #'+ chances))))

    ;; go through all the chances, keeping the sum so far
    (loop for w in chances
          with choice = 0
          summing w into running-sum
...
(defun random-choice (chances-alist)
  "choose one option from an alist of chances, returning its key"
  (car (nth (random-choice-index (mapcar #'cdr chances-alist)) chances-alist)))

(defun from-dungeon-level (alist)
  "returns a value that depends on level. the alist specifies what value occurs after each level,
   default is 0."
  (loop for (value . level) in (reverse alist)
...
(defun place-objects (room)
  ...
  (let ((max-monsters (from-dungeon-level '((2 . 1) (3 . 4) (5 . 6))))
    ...
    (dotimes (i (tcod:random-get-int tcod:+null+ 0 max-monsters))
      ...
      (unless (is-blocked x y)
        (let ((choice (random-choice monster-chances)))
          (cond ((eql choice :orc)
                 ;; create an orc
  ...
  (let ((max-items (from-dungeon-level '((1 . 1) (2 . 4))))
    ...
    (setf *objects*
          ...
          (append (loop for i below (tcod:random-get-int tcod:+null+ 0 max-items)
                        ...
                        unless (is-blocked x y)
                          if (eql choice :heal)
                            collect (make-instance 'item :x x :y y :cha #\!
                                                         ...
                                                         :color :purple
                                                         :always-visible t)
                        else if (eql choice :lightning)
                               collect (make-instance 'item :x x :y y :cha #\#
                                                            :name "scroll of lightning bolt"
                                                            :use-function #'cast-lightning
                                                            :color :light-yellow
                                                            :always-visible t)
                        else if (eql choice :fireball)
                               collect (make-instance 'item :x x :y y :cha #\#
                                                            :name "scroll of fireball"
                                                            :use-function #'cast-fireball
                                                            :color :light-yellow
                                                            :always-visible t)
                        else if (eql choice :confuse)
                               collect (make-instance 'item :x x :y y :cha #\#
                                                            :name "scroll of confusion"
                                                            :use-function #'cast-confuse
                                                            :color :light-yellow
                                                            :always-visible t)
                        else if (eql choice :sword)
                               collect (make-instance 'equipment :x x :y y :cha #\/
                                                                 :name "sword"
                                                                 :equip-slot :right-hand
                                                                 :power-bonus 3
                                                                 :color :blue
                                                                 :always-visible t)
                        else if (eql choice :shield)
                               collect (make-instance 'equipment :x x :y y :cha #\[
                                                                 :name "shield"
...
(defun new-game ()
  ...
  (setf *player* (make-instance 'fighter
                                ...
                                :max-hp 100 :defense 1 :power 2 :xp 0 :level 1
  ...
  (push (make-instance 'equipment :x 0 :y 0 :cha #\- :name "dagger" :equip-slot :right-hand
                                  :power-bonus 2 :color :blue :always-visible t) *inventory*)
  (equip (car *inventory*)))
