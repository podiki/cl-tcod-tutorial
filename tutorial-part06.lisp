;;;; tutorial-part06.lisp
;;;; "Going Berserk!"
;;;; February 2017
;;;;
;;;; Commentary to be posted on 9bladed.com
;;;;
;;;; Original python+libtcod tutorial, part 6:
...
(defmethod attack ((self fighter) target)
  ...
  (let ((damage (- (power self) (defense target))))
    (if (> damage 0)
        ...
        (progn (format t "~&~@(~a~) attacks ~a for ~a hit points." (name self) (name target) damage)
               (take-damage target damage))
        (format t "~&~@(~a~) attacks ~a but it has no effect!" (name self) (name target)))))
...
(defun monster-death (monster)
  ...
  (format t "~&~@(~a~) is dead!" (name monster))
  (setf (cha monster) #\%
...
(defun render-all ()
  ...
  (tcod:console-set-default-foreground *con* (tcod:color :white))
  (tcod:console-print-ex *con* 1 (- *screen-height* 2) :none :left
                         "HP: ~a / ~a" (hp *player*) (max-hp *player*))
